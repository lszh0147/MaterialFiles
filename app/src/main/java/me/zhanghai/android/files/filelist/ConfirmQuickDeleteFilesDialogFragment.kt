...
class ConfirmQuickDeleteFilesDialogFragment : AppCompatDialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        return MaterialAlertDialogBuilder(requireContext(), theme)
            .setMessage(message)
            .setPositiveButton(android.R.string.ok) { _, _ -> listener.quickDeleteFiles(files) }
    ...
    companion object {
        fun show(files: FileItemSet, fragment: Fragment) {
            ConfirmQuickDeleteFilesDialogFragment().putArgs(Args(files)).show(fragment)
        }
    }
    ...
    interface Listener {
        fun quickDeleteFiles(files: FileItemSet)
    }
}
